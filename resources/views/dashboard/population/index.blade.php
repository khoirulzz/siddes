...
@section('content')
    ...
    <section class="panel">
        ...
        <form method="GET" action="{{ route('dashboard.population-records.index') }}" class="inline-form">
            ...
            <div class="field">
                <label for="q">Cari berdasarkan NIK / No KK</label>
                <input id="q" type="text" name="q" value="{{ $filters['q'] }}" placeholder="Contoh: 3326010204010028">
            </div>
            <button class="btn btn-primary" type="submit">Cari</button>
            <a class="btn btn-secondary" href="{{ route('dashboard.population-records.index') }}">Reset</a>
        </form>

        <div class="tabs" style="margin-top:0.8rem; margin-bottom:0;">
            <a class="tab {{ $selectedHamlet === 'Semua' ? 'active' : '' }}" href="{{ route('dashboard.population-records.index', ['q' => $filters['q']]) }}">Semua Dusun</a>
            @foreach($hamlets as $hamlet)
                <a
                    class="tab {{ $selectedHamlet === $hamlet ? 'active' : '' }}"
                    href="{{ route('dashboard.population-records.index', ['hamlet' => $hamlet, 'q' => $filters['q']]) }}"
                >
                    {{ $hamlet }}
                </a>
            @endforeach
