...
<section class="panel" @if($campaign['status']==='RUNNING') data-refresh-seconds="15" @endif>
    <div class="toolbar"><div><h2>{{ $campaign['name'] }}</h2><p><span class="messaging-status">{{ \App\Support\MessagingLabels::status($campaign['status']) }}</span> {{ $campaign['recipientCount'] }} penerima · {{ $campaign['batchSize'] }} pesan per batch</p></div><a class="btn btn-secondary" href="{{ route('dashboard.messaging.campaigns') }}">Kembali</a></div>
    ...
    <div class="messaging-section">
        <div class="actions">
            ...
            <form method="POST" action="{{ route('dashboard.messaging.campaigns.action', [$campaign['id'], 'cancel']) }}" data-confirm="Batalkan campaign? Pesan yang sudah direlay tidak dapat ditarik kembali.">@csrf<button class="btn btn-danger">Batalkan campaign</button></form>
            @endif
            <a class="btn btn-secondary" href="{{ url()->full() }}">Segarkan</a>
        </div>
        @if($campaign['status']==='DRAFT')<p class="muted">Draft belum dikirim. Pilih Mulai pengiriman untuk diproses pada jadwal dispatcher berikutnya.</p>@endif
    </div>
</section>
<section class="panel">
