<div class="messaging-table"><table><thead><tr><th>Penerima</th><th>Pesan</th><th>Proses</th><th>Pengantaran</th><th>Aksi</th></tr></thead><tbody>
    ...
    <td>@if($item['status']==='FAILED' || ($item['status']==='SENT' && in_array($item['deliveryStatus'], ['ERROR','UNKNOWN'])))
        <form method="POST" action="{{ route('dashboard.messaging.messages.retry', $item['id']) }}" data-confirm="{{ $item['deliveryStatus']==='UNKNOWN' ? 'Pengiriman belum pasti. Mengirim ulang dapat menyebabkan pesan ganda. Lanjutkan?' : 'Antrekan ulang pesan ini?' }}">@csrf<button class="btn btn-secondary">Antrekan ulang</button></form>
        @else<span class="muted">—</span>@endif
    </td></tr>
    @empty<tr><td colspan="5" class="messaging-empty">Belum ada pesan yang cocok.</td></tr>@endforelse
</tbody></table></div>
<p class="muted">Diserahkan belum berarti pesan diterima penerima. Pengiriman tidak pasti tidak diulang otomatis; periksa penerima sebelum mengantrekan ulang.</p>
